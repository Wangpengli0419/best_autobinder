...
class TypeMapper {
    /** 短类型 -> 完整类型 */
    static toFull(type) {
        return Const_1.default.SeparatorMap[type] || type;
    }
    /** 完整类型 -> 短类型 */
    static toShort(full) {
    ...
    /** 获取组件查找的所有可能类型名 */
    static getComponentTypes(full) {
        const types = [];
        const parts = full.split('.');
        if (parts.length === 2)
            types.push(parts[1], full);
        else if (parts.length >= 3)
            types.push(parts.slice(1).join('.'), full);
        else
            types.push(full);
...
_a = TypeMapper;
// 预计算映射表
TypeMapper.fullToShort = new Map(); // cc.Label -> Label (用于 import 和 property)
TypeMapper.fullToProperty = new Map(); // cc.Label -> Label
TypeMapper.fullToImport = new Map(); // cc.sp.Skeleton -> sp
TypeMapper.ccTypeToKeys = new Map(); // cc.Node -> [Node, autoNode, MyNode]
TypeMapper.typeToKey = new Map(); // ToggleContainer -> ToggleGroup (反向映射)
(() => {
    // 构建映射表
    const entries = Object.entries(Const_1.default.SeparatorMap);
    for (const [key, full] of entries) {
        const parts = full.split('.');
        // full -> short (用于 import，始终使用内置短类型)
        const builtinShort = parts.length >= 3 ? `${parts[1]}.${parts[2]}` : parts[1];
        _a.fullToShort.set(full, builtinShort);
        // full -> property type
        const propType = parts.length >= 3 ? `${parts[1]}.${parts[2]}` : parts[1];
        _a.fullToProperty.set(full, propType);
        // full -> import module
        if (parts[1])
            _a.fullToImport.set(full, parts[1]);
        // ccType -> keys (同 ccType 的所有 key)
        if (!_a.ccTypeToKeys.has(full))
            _a.ccTypeToKeys.set(full, []);
        _a.ccTypeToKeys.get(full).push(key);
        // 反向映射: property type -> key
        _a.typeToKey.set(propType, key);
    }
})();
// ============= 属性解析服务 =============
class PropertyParser {
    ...
    /** 解析单个装饰器节点 */
    static parseDecorator(node) {
        const expression = node.expression;
        // 只处理 @property 装饰器
        if (!ts.isCallExpression(expression))
            return null;
        const callee = expression.expression;
        if (!ts.isIdentifier(callee) || callee.getText() !== 'property')
            return null;
        // 解析装饰器参数
    ...
    static async parseFromFile(url) {
...
class NodeFinder {
    ...
    static getByPath(root, path) {
        ...
        for (const p of parts) {
            if (!node)
                return null;
            node = (_b = node.children) === null || _b === void 0 ? void 0 : _b.find((c) => c.name === p);
        }
        return node;
    }
    // 获取节点上自定义组件的导入路径
    static async getComponentPath(root, nodeName, componentName) {
    ...
    // 同步版本：查找节点上的自定义组件
    static findComponentScript(root, nodeName, componentName) {
        const node = this.getByName(root, nodeName);
        if (!node)
            return null;
        for (const comp of node.components || []) {
    ...
    static findDeep(node, predicate) {
        if (predicate(node))
            return node;
    ...
    static getCustomScripts(node) {
        return (node.components || [])
            .filter((c) => c.__scriptUuid)
            .map((c) => ({
    ...
    static getComponentIndex(node, script) {
        const comps = node._components || [];
        return comps.findIndex((c) => c.constructor.name === script.__classname__);
    }
    // 检查节点上是否存在指定名称的组件
    static hasComponent(node, componentName) {
        for (const comp of node.components || []) {
            if (comp.constructor.name === componentName) {
                return true;
            }
...
class NodeNameParser {
    static parse(name, isRoot = false) {
        // 根节点不做解析
        if (isRoot) {
            return null;
        }
        ...
        // 1. 以 _ 开头（私有属性）
        // 2. 不包含 @ 分隔符
        if (name.startsWith('_') || !name.includes(Const_1.default.STANDARD_Separator))
            return null;
        const [type, rest] = name.split(Const_1.default.STANDARD_Separator);
        if (!type || !rest)
            return null;
        // 原始属性名（用于节点查找）
        // 如果以 # 结尾，去掉 # 后缀
        let nodeProp = rest.endsWith(Const_1.default.STANDARD_End) ? rest.slice(0, -1) : rest;
    ...
    static getCCType(type) {
        // 先检查是否是 SeparatorMap 的 key
        if (Const_1.default.SeparatorMap[type])
            return Const_1.default.SeparatorMap[type];
        // 再检查是否是 ccType 的后缀
        for (const key of Object.keys(Const_1.default.SeparatorMap)) {
            if (Const_1.default.SeparatorMap[key].endsWith('.' + type))
                return Const_1.default.SeparatorMap[key];
        }
    ...
    static isBuiltinType(type) {
        ...
        if (Const_1.default.SeparatorMap[type])
            return true;
        for (const key of Object.keys(Const_1.default.SeparatorMap)) {
            if (Const_1.default.SeparatorMap[key] === type)
                return true;
            if (Const_1.default.SeparatorMap[key].endsWith('.' + type))
                return true;
        }
    ...
    static findCandidates(node, existing, isRoot = false) {
    ...
    /** 检查节点名称是否以 # 结尾 */
    static hasEndMark(nodeName) {
        return nodeName.endsWith(Const_1.default.STANDARD_End);
    }
}
// ============= 脚本修改服务 =============
class ScriptModifier {
    // 添加自定义组件的 import，需要路径
    static addCustomImport(content, componentPath, componentName) {
        var _b;
        // 检查是否已有 import
        ...
        const pos = (_b = content.match(/^import\s+/m)) === null || _b === void 0 ? void 0 : _b.index;
        if (pos !== undefined) {
            return content.slice(0, pos) + importStatement + content.slice(pos);
        }
        return importStatement + content;
    }
    static ensureBuiltinImports(content, types) {
        const ccTypes = [];
        ...
        for (const type of types) {
            ...
            let fullType;
            for (const [key, full] of Object.entries(Const_1.default.SeparatorMap)) {
                const parts = full.split('.');
                const builtinShort = parts.length >= 3 ? `${parts[1]}.${parts[2]}` : parts[1];
    ...
    static addCCImport(content, parts) {
        var _b;
    ...
    static insertProperty(content, decl) {
...
class SceneBinder {
    static async bindProperty(root, script, prop) {
        ...
        if (!value) {
            // 输出节点上所有组件名称帮助调试
            const compNames = (target.components || []).map((c) => c.constructor.name);
    ...
    static async setPropertyDump(root, script, prop, ccType) {
        const compIdx = NodeFinder.getComponentIndex(root, script);
        if (compIdx < 0)
            return;
    ...
    static async setPropertyDumpWithType(uuid, path, type, targetUuid) {
    ...
    static findNode(root, keys, prop) {
        for (const key of keys) {
            // 原始属性名
            let node = NodeFinder.getByName(root, `${key}@${prop}`);
            if (!node)
                node = NodeFinder.getByName(root, `${key}@${prop}${Const_1.default.STANDARD_End}`);
            if (node)
                return node;
            ...
            if (!node)
                node = NodeFinder.getByName(root, `${key}@${propWithDash}${Const_1.default.STANDARD_End}`);
            if (node)
                return node;
            ...
            if (node)
                return node;
            node = NodeFinder.getByName(root, `${prop}${Const_1.default.STANDARD_End}`);
            if (node)
                return node;
        }
        return null;
    }
    static findComponent(node, types) {
        // 遍历节点组件匹配
        for (const comp of node.components || []) {
            const compName = comp.constructor.name;
            if (types.includes(compName))
                return comp;
        }
        // 回退到 getComponent
    ...
    static validateCustomCandidates(root, candidates) {
        const valid = [];
        const invalid = [];
    ...
    // 为自定义组件添加 import 语句
    static async addCustomImports(root, content, candidates) {
        for (const c of candidates) {
    ...
    static generateNewProperties(content, candidates) {
        const props = [];
        for (const c of candidates) {
    ...
    // 绑定新创建的属性到场景
    static async bindNewProperties(root, comp, candidates) {
    ...
    // 查找候选节点
    static findCandidateNodes(root, propNames) {
        const candidates = NodeNameParser.findCandidates(root, propNames, true);
        if (!candidates.length)
            return [];
        // 过滤掉已存在的属性名
        const newCandidates = candidates.filter((c) => !propNames.includes(c.prop));
        logger(`待声明节点 : ${newCandidates.map(c => c.nodeName).join(', ')}`);
        return newCandidates;
    }
    // 读取脚本文件内容
    static readScriptContent(url) {
        if (!url)
            return null;
        const path = url.replace('db://', Editor.Project.path + '/').replace(/\\/g, '/');
    ...
    // 分离内置类型和自定义组件
    static separateCandidateTypes(candidates) {
        const builtin = candidates.filter((c) => c.isBuiltin);
        const custom = candidates.filter((c) => !c.isBuiltin);
        return { builtin, custom };
    }
    // 处理内置类型的 import
    static processBuiltinImports(content, builtinCandidates) {
        const builtinTypes = [...new Set(builtinCandidates.map((c) => TypeMapper.toShort(c.ccType)))];
        return ScriptModifier.ensureBuiltinImports(content, builtinTypes);
    }
    // 生成新属性声明
    static createNewProperties(content, candidates) {
    ...
    // 保存脚本文件
    static saveScriptContent(url, content) {
        const path = url.replace('db://', Editor.Project.path + '/').replace(/\\/g, '/');
        (0, fs_extra_1.writeFileSync)(path, content, 'utf-8');
    }
    // 重新导入脚本资源
    static async reimportScript(uuid) {
        try {
    ...
    // 重新获取组件实例
    static getReimportedComponent(root, scriptName) {
        return root.getComponent(scriptName);
    }
    static async scanAndCreate(root, script, existing) {
        // 1. 获取脚本路径
        const url = await Editor.Message.request('asset-db', 'query-url', script.uuid);
        if (!url)
            return [];
        // 2. 获取属性名列表
        const propNames = existing.map((p) => p.name);
        // 3. 查找候选节点
...
exports.methods = {
    async bind(...args) {
        const root = cc_1.director.getScene();
        let ids = [];
    ...
    // 递归处理节点及其所有后代节点
    async processNodeAndDescendants(node, bound, created) {
        // 处理当前节点的脚本
        for (const script of NodeFinder.getCustomScripts(node)) {
            const url = await Editor.Message.request('asset-db', 'query-url', script.uuid);
            const props = url ? await PropertyParser.parseFromFile(url) : [];
